...
func Test_spotifyAuthService_StartAuthFlow(t *testing.T) {
	state := "abc123"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Start the auth flow
			gotURL, err := svc.StartAuthFlow(ctx)
			tt.wantErr(t, err)
			...
			assert.NoError(t, svc.server.Shutdown(shutdownCtx))
		})
	}
}
